...
const checkHttp = async (domain) => {
  ...
  try {
    ...
    if (response.status >= 200 && response.status < 300) {
      return { ok: true, reason: `HTTP ${response.status} - Site is live`, latency };
    }
    
    // 3xx redirects are followed automatically, but if we get here it's an issue
    if (response.status >= 300 && response.status < 400) {
      return { ok: true, reason: `HTTP ${response.status} - Redirect (site accessible)`, latency };
    }
    
    // 4xx and 5xx mean the site is not working properly
    return { ok: false, reason: `HTTP ${response.status} - ${response.statusText || "Error"}`, latency };
  } catch (error) {
    ...
    if (url.startsWith("https://") && error.name !== "AbortError") {
      try {
        ...
        if (response.status >= 200 && response.status < 300) {
          return { ok: true, reason: `HTTP ${response.status} - Site is live (HTTP only)`, latency };
        }
        
        return { ok: false, reason: `HTTP ${response.status} - ${response.statusText || "Error"}`, latency };
      } catch (httpError) {
        return { ok: false, reason: `Connection failed: ${normalizeError(error)}`, latency: null };
      }
    }
